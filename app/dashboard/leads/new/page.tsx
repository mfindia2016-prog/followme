    ...
    try {
      ...
      if (nextFollowUpDate) {
        ...
        if (!Number.isNaN(localDate.getTime())) {
          nextFollowupAt = localDate.toISOString();
        }
      }
      ...
      if (assignedAgent) {
        const { data: selectedAgent, error: agentError } =
          await supabase
            .from("agent_profiles")
        ...
        if (agentError) {
          setError("Unable to verify selected agent: " + agentError.message);
          setSaving(false);
          return;
        }
        ...
        if (!selectedAgent) {
          setError(
            "Selected agent was not found. Please select an active agent again."
          );
          setSaving(false);
          return;
        }
